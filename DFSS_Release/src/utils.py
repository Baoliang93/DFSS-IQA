...
class Metric(metaclass=ABCMeta):
    ...
    def compute_rmse_wo_fit(self):
        x1_array = np.array(self.x1.copy(), dtype=np.float64)
        x2_array = np.array(self.x2.copy(), dtype=np.float64)
        zz = np.sqrt(((x2_array - x1_array) ** 2).mean())
        return zz
    ...
    def compute(self):
        mos = np.array(self.x1, dtype=np.float64).flatten()/self.scale 
        obj_score = np.array(self.x2, dtype=np.float64).flatten()/self.scale 
        beta1 = np.max(mos)
        beta2 = np.min(mos)
        ...
        fit_stat = ''
        try:
            popt, _ = scipy.optimize.curve_fit(self.logistic, xdata=obj_score, ydata=mos, p0=beta, maxfev=100000)
        except:
            popt = beta
...
#     def compute(self):
#         x1_array = np.array(self.x1, dtype=np.float64)
#         x2_array = np.array(self.x2, dtype=np.float64)
